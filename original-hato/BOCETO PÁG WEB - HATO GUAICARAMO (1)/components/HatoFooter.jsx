/* HatoFooter — beige bg · logo+intro on left · contact+social on right · photo strip bottom */
  ...
  return (
    <footer id="contacto" style={{ background: "var(--g-beige)", color: "var(--g-fg)", position: "relative" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.05fr 1fr", gap: 80, alignItems: "start" }}>
          <div>
            ...
            <p style={{
              ...
              color: "var(--g-fg)", margin: 0, maxWidth: 480, textWrap: "pretty", fontFamily: "\"Atlas Grotesk\""
            }}>
      ...
      <div style={{
        ...
        <img src="assets/photography/manada-footer.jpg" alt="Manada de búfalos y Nelore del Hato Guaicaramo" style={{
          display: "block",
      ...
      <div style={{
        ...
        <span>·</span>
        <a href="assets/pdf/Politica-Tratamiento-Datos.pdf" download style={{
          color: "var(--g-fg-muted)",
          textDecoration: "none",
          transition: "color 160ms var(--g-ease-soft)",
          cursor: "pointer"
        }} onMouseEnter={(e) => e.target.style.color = "var(--g-verde-500)"} onMouseLeave={(e) => e.target.style.color = "var(--g-fg-muted)"}
...
function ContactRow({ icon, lines }) {
  return (
...
function SocialCircle({ url, children, title }) {
  const [h, setH] = React.useState(false);
  return (
    <a href={url} target="_blank" rel="noopener" title={title} aria-label={title}
    onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
    style={{
