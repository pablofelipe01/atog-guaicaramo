/* NUESTRAS BUENAS PRÁCTICAS — módulo distinto: liderado por ILUSTRACIONES ANIMADAS.
   Cada práctica tiene un gráfico SVG en movimiento (rueda de pastoreo, riego con gotas,
   escudo con latido, ADN + medidor de preñez). Filas alternas conectadas por una línea
   que se dibuja al hacer scroll. Paleta verde + café cálido sobre beige.
   Patrón de animación: reveal-on-scroll, word-rise, draw-on, loops continuos. */
...
function BPReveal({ children, delay = 0, as = "div", y = 18, dur = 760, style }) {
...
/* ---------- Word-rise heading ---------- */
function BPRise({ text, delay = 0, color, italic = false, size, weight = 400 }) {
  const [ref, seen] = useBPReveal();
  const words = text.split(" ");
  return (
    <div ref={ref} style={{
      fontFamily: "var(--g-font-display)", fontSize: size || "clamp(40px, 6.4vw, 96px)",
      lineHeight: 1.04, letterSpacing: "-0.022em", color: color || "var(--g-verde-900)",
      fontStyle: italic ? "italic" : "normal", fontWeight: weight, margin: 0,
    }}>
      {words.map((w, wi) => (
        <span key={wi} style={{
          display: "inline-block", clipPath: "polygon(-100% 0, 200% 0, 200% 100%, -100% 100%)",
          verticalAlign: "top", paddingTop: "0.32em", paddingBottom: "0.32em",
          marginTop: "-0.32em", marginBottom: "-0.32em", lineHeight: 1.04,
          marginRight: wi === words.length - 1 ? 0 : "0.28em",
        }}>
          <span style={{
            display: "inline-block",
            transform: seen ? "translateY(0) skewY(0)" : "translateY(112%) skewY(6deg)",
            opacity: seen ? 1 : 0, filter: seen ? "blur(0)" : "blur(6px)",
...
/* ---------- Brand-color palettes (4 colores del sistema) ---------- */
const PAL = {
  verde:      { main: "#627761", dark: "#3e4c40", light: "#9aad99", soft: "#f1f3ef", line: "#dde2d8",
                wheel: ["#9aad99", "#7e9078", "#627761", "#4f6151", "#869a82", "#3e4c40"] },
  petroleo:   { main: "#233653", dark: "#1a2840", light: "#647394", soft: "#e9edf2", line: "#ccd4e0",
                wheel: ["#97a4ba", "#647394", "#3d4f72", "#2c3f5f", "#7c8aa6", "#233653"] },
  cafe:       { main: "#a4986f", dark: "#6f664a", light: "#cdc3a3", soft: "#f7f3e6", line: "#e8e0c9",
                wheel: ["#ddd4b6", "#cdc3a3", "#bcb088", "#a4986f", "#d4cbac", "#8a7f5c"] },
  verdeClaro: { main: "#9aad99", dark: "#5d7059", light: "#bccbbb", soft: "#eef2ed", line: "#d8e0d6",
                wheel: ["#cdd8cc", "#bccbbb", "#9aad99", "#869a82", "#aab9a6", "#7e9078"] },
};

/* =====================================================================
   ILUSTRACIÓN 01 · Rueda de pastoreo rotacional
===================================================================== */
function BPPaddockWheel({ c = PAL.verde }) {
  const [ref, seen] = useBPReveal();
      ...
      <svg viewBox="0 0 200 200" style={{ width: "100%", height: "auto", overflow: "visible" }}>
        <g style={{ transformOrigin: "100px 100px", animation: seen ? "bp-spin 40s linear infinite" : "none" }}>
          {Array.from({ length: sectors }).map((_, i) => (
            <path key={i} d={wedge(i)} fill={greens[i]} stroke="var(--g-beige)" strokeWidth="2"
              style={{ opacity: seen ? 1 : 0, transition: `opacity 500ms ease ${i * 90}ms` }} />
          ))}
        </g>
        {/* center hub */}
...
/* =====================================================================
   ILUSTRACIÓN 02 · Riego — gotas + nivel estable
===================================================================== */
function BPIrrigation({ c = PAL.petroleo }) {
  const [ref, seen] = useBPReveal();
  const heads = [50, 100, 150];
  return (
    <div ref={ref} style={{ width: "100%", maxWidth: 300, margin: "0 auto" }}>
      <svg viewBox="0 0 200 200" style={{ width: "100%", height: "auto", overflow: "visible" }}>
        {/* sky arcs from sprinkler heads */}
        {heads.map((hx, i) => (
          <g key={i}>
            <path d={`M${hx},70 q-26,-26 -50,4`} fill="none" stroke={c.light} strokeWidth="2" strokeLinecap="round" strokeDasharray="3 6"
              style={{ opacity: seen ? 0.85 : 0, transition: `opacity 600ms ease ${i * 140}ms` }} />
            <path d={`M${hx},70 q26,-26 50,4`} fill="none" stroke={c.light} strokeWidth="2" strokeLinecap="round" strokeDasharray="3 6"
              style={{ opacity: seen ? 0.85 : 0, transition: `opacity 600ms ease ${i * 140}ms` }} />
            {/* head post */}
            ...
            {/* droplets */}
            {[0, 1, 2].map((d) => (
              <ellipse key={d} cx={hx} cy={80} rx="2.6" ry="4" fill={c.main}
                style={{ transformOrigin: `${hx}px 80px`, animation: seen ? `bp-drop 1.8s ease-in ${(i * 0.3 + d * 0.5)}s infinite` : "none", opacity: 0 }} />
            ))}
          </g>
        ))}
        {/* ground */}
        ...
        {/* stable level line */}
        <line x1="20" y1="140" x2="180" y2="140" stroke={c.main} strokeWidth="2.5" strokeDasharray="200"
          style={{ strokeDashoffset: seen ? 0 : 200, transition: "stroke-dashoffset 1400ms var(--g-ease-out) 400ms" }} />
        {/* grass blades */}
        {[28, 56, 84, 112, 140, 168].map((gx, i) => (
          <path key={gx} d={`M${gx},164 q-3,-14 0,-22 q3,8 0,22`} fill={c.main}
            style={{ transformOrigin: `${gx}px 164px`, animation: seen ? `bp-sway 3s ease-in-out ${i * 0.2}s infinite` : "none", opacity: seen ? 1 : 0, transition: `opacity 500ms ease ${500 + i * 80}ms` }} />
        ))}
      </svg>
    </div>
  );
...
/* =====================================================================
   ILUSTRACIÓN 03 · Escudo de salud + latido
===================================================================== */
function BPHealthShield({ c = PAL.cafe }) {
  const [ref, seen] = useBPReveal();
  return (
    <div ref={ref} style={{ width: "100%", maxWidth: 300, margin: "0 auto" }}>
      <svg viewBox="0 0 200 200" style={{ width: "100%", height: "auto", overflow: "visible" }}>
        {/* shield */}
        <path d="M100,28 L160,50 V104 C160,140 132,162 100,174 C68,162 40,140 40,104 V50 Z"
          fill={c.soft} stroke={c.main} strokeWidth="3"
          style={{ strokeDasharray: 520, strokeDashoffset: seen ? 0 : 520, transition: "stroke-dashoffset 1500ms var(--g-ease-out) 200ms" }} />
        {/* heartbeat line */}
        <polyline points="56,108 80,108 90,86 104,128 116,108 144,108" fill="none"
          stroke={c.dark} strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round"
          style={{ strokeDasharray: 170, animation: seen ? "bp-beat 2.4s var(--g-ease-soft) infinite" : "none", strokeDashoffset: seen ? undefined : 170 }} />
        {/* plus badge */}
...
===================================================================== */
function BPGeneGauge({ c = PAL.verdeClaro }) {
  const [ref, seen] = useBPReveal();
  ...
  for (let i = 0; i <= 7; i++) {
    const t = i / 7;
    const y = top + t * h;
    const x1 = hx + amp * Math.sin(t * Math.PI * 2 * turns);
    const x2 = hx + amp * Math.sin(t * Math.PI * 2 * turns + Math.PI);
    rungs.push(<line key={i} x1={x1} y1={y} x2={x2} y2={y} stroke={c.light} strokeWidth="1.6"
      style={{ opacity: seen ? 1 : 0, transition: `opacity 400ms ease ${300 + i * 80}ms` }} />);
  }
  const helixPath = (phase) => {
    ...
    <div ref={ref} style={{ width: "100%", maxWidth: 300, margin: "0 auto" }}>
      <svg viewBox="0 0 200 200" style={{ width: "100%", height: "auto", overflow: "visible" }}>
        ...
        <g transform="translate(140,100)">
          ...
          <circle r={R} fill="none" stroke={c.main} strokeWidth="9" strokeLinecap="round"
            transform="rotate(-90) scale(0.62)"
            style={{ strokeDasharray: C, strokeDashoffset: seen ? C * 0.12 : C, transition: "stroke-dashoffset 1700ms var(--g-ease-out) 300ms" }} />
          {/* up arrow */}
          <path d="M0,16 L0,-14 M-9,-5 L0,-15 L9,-5" fill="none" stroke={c.dark} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"
            style={{ opacity: seen ? 1 : 0, transition: "opacity 500ms ease 1300ms" }} />
        </g>
      </svg>
    </div>
  );
...
/* =====================================================================
   MÓDULO
===================================================================== */
function BuenasPracticasModule() {
  HatoLucide();
  pEff(() => { if (window.lucide) window.lucide.createIcons(); });

  const practices = [
    {
      id: "pastoreo", n: "01", tag: "Manejo del forraje", title: "Pastoreo rotacional",
      illo: BPPaddockWheel, icon: "refresh-cw", pal: PAL.verde,
      img: "assets/photography/pastoreo-vaquero.png",
    ...
    {
      id: "riegos", n: "02", tag: "Infraestructura productiva", title: "Nuestros riegos",
      illo: BPIrrigation, icon: "droplets", pal: PAL.petroleo,
      img: "assets/photography/riego-carrete.png",
    ...
    {
      id: "vacuna", n: "03", tag: "Salud del hato", title: "Vacunación y desparasitación",
      illo: BPHealthShield, icon: "shield-plus", pal: PAL.cafe,
    ...
    {
      id: "insem", n: "04", tag: "Biotecnología reproductiva", title: "Inseminación artificial",
      illo: BPGeneGauge, icon: "dna", pal: PAL.verdeClaro,
      img: "assets/photography/inseminacion-embriones.png",
  ...
  return (
    <>
      <style>{`
        ...
        @media (max-width: 920px){ .bp-row{ grid-template-columns: 1fr !important; } .bp-row .bp-illo-col{ order: -1; } }
      `}</style>
      <BPHero />
      <BPIntro />
...
/* =====================================================================
   HERO — ilustración-led (distinto a los demás)
===================================================================== */
function BPHero() {
  pEff(() => { if (window.lucide) window.lucide.createIcons(); });
  const motifs = [
  ...
  return (
    <section style={{
      ...
      background: "linear-gradient(160deg, var(--g-verde-900) 0%, var(--g-verde-800) 52%, var(--g-verde-700) 100%)",
      color: "var(--g-beige)", display: "flex", flexDirection: "column", justifyContent: "center",
    }}>
      {/* faint grid */}
      <div aria-hidden style={{ position: "absolute", inset: 0, opacity: 0.5,
        backgroundImage: "radial-gradient(circle at 50% 0%, rgba(154,173,153,0.18), transparent 60%)" }}/>
      {/* floating illustration motifs (decor) */}
      {motifs.map((m, i) => {
        const Illo = m.C;
        return (
          <div key={i} aria-hidden style={{
            position: "absolute", top: m.top, left: m.left, width: m.size,
            opacity: 0.16, filter: "grayscale(0.2)",
            animation: `bp-float ${m.dur} ease-in-out ${i * 0.6}s infinite`, pointerEvents: "none",
          }}>
            <Illo c={m.c} />
          </div>
        );
      ...
      <div style={{ position: "relative", maxWidth: 1100, margin: "0 auto", width: "100%", padding: "120px 56px 80px", textAlign: "center" }}>
        <BPReveal>
          <div style={{
        ...
        <BPReveal delay={520}>
          <p style={{
            margin: "30px auto 0", maxWidth: "52ch",
            fontFamily: "var(--g-font-sans)", fontSize: "clamp(16px, 1.5vw, 21px)",
            lineHeight: 1.6, color: "rgba(249,246,232,0.88)", textWrap: "pretty",
          }}>
            Producir bien empieza por hacer las cosas bien. Cuatro prácticas que
            sostienen la productividad, la salud y la sostenibilidad del hato.
          </p>
        </BPReveal>
      </div>
      ...
      <div aria-hidden style={{
        ...
        animation: "bp-cueFloat 3s ease-in-out infinite",
        width: 27, height: 43, borderRadius: 999, border: "1.5px solid rgba(249,246,232,0.45)",
...
/* solid-color index card (portada) */
function BPIndexCard({ it, delay }) {
  const [h, setH] = pUse(false);
  pEff(() => { if (window.lucide) window.lucide.createIcons(); });
  return (
    <BPReveal delay={delay}>
      <a href={"#" + it.t}
        ...
        <div style={{
          position: "relative", background: it.c.main, borderRadius: 22, padding: "24px 24px 28px",
          minHeight: 188, display: "flex", flexDirection: "column", overflow: "hidden",
          transform: h ? "translateY(-6px)" : "none",
          boxShadow: h ? "0 22px 48px -16px rgba(26,33,32,0.4)" : "0 8px 22px -12px rgba(26,33,32,0.28)",
          ...
          <span style={{ display: "inline-flex", width: 46, height: 46, borderRadius: 999, background: "rgba(249,246,232,0.16)", color: "var(--g-beige)", alignItems: "center", justifyContent: "center" }}>
            <i data-lucide={it.icon} style={{ width: 21, height: 21 }}/>
          </span>
          {/* title */}
...
function BPIntro() {
  ...
  pEff(() => { if (window.lucide) window.lucide.createIcons(); });
  return (
    <section style={{ background: "var(--g-bg)", padding: "clamp(72px,9vw,120px) 0 clamp(40px,5vw,64px)", position: "relative", overflow: "hidden" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 56px" }}>
        <div className="bp-row" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "clamp(32px,5vw,72px)", alignItems: "end", marginBottom: "clamp(40px,5vw,64px)" }}>
          <div>
            <BPReveal>
          ...
          <BPReveal delay={160}>
            <p style={{ fontFamily: "var(--g-font-sans)", fontSize: 16, lineHeight: 1.7, color: "var(--g-cafe-700)", margin: 0, maxWidth: "46ch", textWrap: "pretty" }}>
              Cada práctica se planifica, se mide y se ajusta. No son rutinas sueltas:
              forman un sistema que se sostiene entre sí.
            </p>
          </BPReveal>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "clamp(14px,1.6vw,22px)" }}>
          {idx.map((it, i) => (
            <BPIndexCard key={it.n} it={it} delay={i * 90} />
          ))}
        </div>
      </div>
    </section>
...
===================================================================== */
function BPPractice({ id, n, tag, title, illo, icon, lead, chips, note, flip, last, pal = PAL.verde, img }) {
  const Illo = illo;
  const c = pal;
  const [ref, seen] = useBPReveal();
  pEff(() => { if (window.lucide) window.lucide.createIcons(); });
  return (
    <div id={"bp-" + n} ref={ref} style={{ position: "relative", paddingTop: "clamp(40px,6vw,72px)", paddingBottom: last ? 0 : "clamp(40px,6vw,72px)" }}>
      <div className="bp-row" style={{ display: "grid", gridTemplateColumns: flip ? "1fr 1.05fr" : "1.05fr 1fr", gap: "clamp(36px,5vw,80px)", alignItems: "center" }}>
        {/* Illustration column */}
        <div className="bp-illo-col" style={{ order: flip ? 2 : 1 }}>
          <BPReveal>
            {img ? (
              <div style={{
                position: "relative", borderRadius: 24, overflow: "hidden", aspectRatio: "1 / 1",
                border: `1px solid ${c.line}`, boxShadow: "0 20px 50px -20px rgba(26,33,32,0.28)",
              }}>
                <img src={img} alt={title} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                {/* brand-color wash + number */}
                ...
                <span style={{ position: "absolute", bottom: 18, left: 22, display: "inline-flex", alignItems: "center", gap: 9, padding: "7px 14px", borderRadius: 999, background: c.main, color: "var(--g-beige)", fontFamily: "var(--g-font-sans)", fontSize: 12.5, fontWeight: 600, letterSpacing: "0.04em" }}>
                  <i data-lucide={icon} style={{ width: 15, height: 15 }} /> {tag}
                </span>
              </div>
            ) : (
              <div style={{
                position: "relative", background: c.soft, border: `1px solid ${c.line}`,
                borderRadius: 24, padding: "clamp(28px,4vw,48px)", aspectRatio: "1 / 1",
                display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden",
                boxShadow: "0 20px 50px -20px rgba(26,33,32,0.2)",
              }}>
                {/* big ghost number */}
        ...
        <div style={{ order: flip ? 1 : 2 }}>
          ...
          <BPReveal delay={140}>
            <p style={{ fontFamily: "var(--g-font-sans)", fontSize: "clamp(16px,1.5vw,19px)", lineHeight: 1.65, color: "var(--g-cafe-700)", margin: "18px 0 22px", maxWidth: "52ch", textWrap: "pretty" }}>
              {lead}
            </p>
          </BPReveal>
          <BPReveal delay={220}>
          ...
          <BPReveal delay={300}>
            <div style={{ display: "flex", gap: 14, alignItems: "flex-start", paddingLeft: 16, borderLeft: `2px solid ${c.light}` }}>
              <p style={{ fontFamily: "var(--g-font-sans)", fontSize: 15, lineHeight: 1.6, color: c.dark, fontStyle: "italic", margin: 0, maxWidth: "50ch", textWrap: "pretty" }}>
                {note}
              </p>
            </div>
          </BPReveal>
...
function BPCTA() {
  return (
    <section style={{ position: "relative", background: "var(--g-verde-900)", color: "var(--g-beige)", padding: "clamp(80px,10vw,130px) 0", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", maxWidth: 1000, margin: "0 auto", padding: "0 56px", textAlign: "center" }}>
        ...
        <BPReveal delay={260}>
          <div style={{ marginTop: 38, display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
            <BPCtaLink href="index.html" solid>Volver al inicio</BPCtaLink>
            <BPCtaLink href="genetica-talla-mundial.html">Genética de talla mundial</BPCtaLink>
            <BPCtaLink href="nutricion-animal.html">Nutrición animal</BPCtaLink>
          </div>
        </BPReveal>
      </div>
    </section>
...
function BPCtaLink({ href, children, solid = false }) {
  const [h, setH] = pUse(false);
  const base = {
  ...
  const solidStyle = { background: h ? "#ffffff" : "var(--g-beige)", color: "var(--g-verde-800)", transform: h ? "translateY(-2px)" : "none", boxShadow: h ? "0 14px 30px rgba(8,16,12,0.34)" : "0 6px 16px rgba(8,16,12,0.22)" };
  const outlineStyle = { background: h ? "rgba(249,246,232,0.12)" : "transparent", color: "var(--g-beige)", border: "1px solid rgba(249,246,232,0.42)" };
  return (
    <a href={href} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)} style={{ ...base, ...(solid ? solidStyle : outlineStyle) }}>{children}</a>
  );
}

window.BuenasPracticasModule = BuenasPracticasModule;
