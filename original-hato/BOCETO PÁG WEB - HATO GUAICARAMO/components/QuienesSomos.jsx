...
function MisionVisionCard({ icon, title, body, delay }) {
  const [h, setH] = React.useState(false);
  const initial = title.charAt(0); // M / V — used as warm background letter
  return (
    <div onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
    style={{
      ...
      {/* Warm aura — radial glow on hover */}
      <span aria-hidden="true" style={{
      ...
      {/* Huge serif initial — warm background letter */}
      <span aria-hidden="true" style={{
      ...
      <div style={{ position: "relative" }}>
        {/* Icon with glowing ring on hover */}
        <span style={{
          position: "relative",
          width: 56, height: 56, borderRadius: 999,
          background: h ? "var(--g-verde-500)" : "var(--g-verde-50)",
        ...
        {/* Title — large serif with italic accent */}
        <h3 style={{
          fontFamily: "var(--g-font-display)",
